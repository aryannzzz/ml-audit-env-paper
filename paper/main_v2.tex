...
data leakage and methodological violations have been identified in hundreds
of published papers spanning scientific fields from health sciences to
economics. Existing detection tools rely on static code analysis or
post-hoc manual checklists and do not scale to the interactive,
cross-artifact reasoning that real auditing demands. We introduce
\textsc{MLAuditBench}, the first reinforcement learning environment that
frames machine learning experiment integrity auditing as an interactive
agent task. Agents must sequentially inspect experiment artifacts,
identify violations with evidence-grounded reasoning, and submit a
verdict within a step budget. The environment implements 8 violation
types drawn from the reproducibility literature, spans 5 machine
learning archetypes and 62 unique experiments across 3 difficulty tiers,
and uses a layered reward function combining violation detection
(80 percent), audit efficiency (10 percent), and verdict accuracy
(10 percent). Anti-gaming mechanisms include adversarially clean
experiments and red herrings in harder tasks. We evaluate three frontier
OpenAI models over 10 seeds each, an interactive Claude Sonnet 4.6
expert audit, a preliminary open-weight Qwen2.5-72B baseline, and a
10-seed machine-learning-trained human baseline, all evaluated under
identical grading conditions. GPT-4.1-mini matches or exceeds the human
baseline across all tiers (0.900, 0.901, 0.819 versus 0.82, 0.86, 0.57);
o4-mini exhibits model-level hallucination failures that place it below
the human anchor; all automated agents remain below the interactive
expert ceiling of 0.954, confirming the benchmark discriminates at
frontier capability levels. An evidence-matching threshold ablation
across five thresholds confirms model rankings are stable throughout.
The environment is deployed as an OpenEnv-compatible Docker container
and is fully reproducible from released seeds.
\end{abstract}
...
models and Shapley-value credit assignment inform our partial credit mechanisms.
...
were used.
Ten participants completed each difficulty tier independently
($N{=}30$ total); no participant completed more than one tier.
...
experiment IDs per tier across the seed range.
We recommend applying this protocol using \texttt{scripts/run\_evaluation.py},
which supports stratified seed filtering via the \texttt{--stratified} flag.
...
Mitigations: full open release under CC~BY~4.0, public anti-gaming mechanisms,
...
% ============================================================================
\section{Experiment Pool Distribution}
\label{app:pool}

Table~\ref{tab:pool} shows the distribution of violation-type occurrences
across archetypes in the v1.1 pool (62 experiments total: 12 easy,
12 medium, 16 hard, 20 adversarially clean).
Each single-violation experiment (easy tier) contributes one count;
two-violation experiments (medium tier) contribute to two rows;
three-violation hard-tier episodes and two-violation compound episodes
contribute accordingly.
Clean experiments (drawn at runtime with probability
$\{0.50, 0.35, 0.20\}$ for easy/medium/hard) are not listed.
V2 (Temporal Shuffle) applies only to time-series experiments.

\begin{table}[h]
\centering
\caption{Experiment pool violation-type distribution by archetype (v1.1,
62 experiments). Cell values are the number of experiments that contain
the row violation type in that archetype. Hard-tier compound episodes
contribute to multiple rows. Counts derived directly from
\texttt{generator.py}.}
\label{tab:pool}
\small
\begin{tabular}{lcccccr}
\toprule
\textbf{Violation} & \textbf{tab\_clf} & \textbf{ts\_reg} & \textbf{tab\_multi} & \textbf{tab\_surv} & \textbf{text\_clf} & \textbf{Total} \\
\midrule
V1 Preprocessing Leakage & 6 & 3 & 2 & 2 & 4 & 17 \\
V2 Temporal Shuffle       & 0 & 6 & 0 & 0 & 0 &  6 \\
V3 Target Leakage         & 3 & 0 & 2 & 3 & 3 & 11 \\
V4 Train/Test Overlap     & 1 & 1 & 3 & 2 & 0 &  7 \\
V5 Cherry-Picking         & 4 & 3 & 2 & 2 & 2 & 13 \\
V6 Metric Shopping        & 3 & 0 & 6 & 3 & 2 & 14 \\
V7 Entity Leakage         & 1 & 2 & 2 & 4 & 0 &  9 \\
V8 Multi-Test Leakage     & 4 & 0 & 0 & 2 & 1 &  7 \\
\midrule
\textbf{Column total}     & 22 & 15 & 17 & 18 & 12 & \textbf{84} \\
\bottomrule
\end{tabular}
\end{table}

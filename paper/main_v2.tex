...
(\texttt{all\_results.txt} in the repository); and (f) identifies and
...
\item \textbf{Residual inference-script variance.}  Addressed:
five additional failure modes (e)--(i) in
\texttt{inference.py} were identified via per-seed root-cause analysis
and fixed (Section~\ref{sec:fixes}).  GPT-4.1-mini easy/medium 10-seed
variance dropped from $\sigma{=}0.39/0.34$ (v1 script) to
$\sigma{=}0.00/0.003$ (v2 script), confirming that the v1 mean scores
were substantially depressed by fixable script bugs rather than model
capability limits.  All raw per-seed v1 and v2 scores are in
\texttt{all\_results.txt} (Sections~1 and~10).
\end{enumerate}
...
threshold ablation, bootstrap CIs, a stratified-seed protocol, five
additional inference-script fixes that eliminate easy/medium evaluation
variance, and a self-critique mapping every prior weakness to its
remediation.  Frontier LLMs already match or exceed humans on
cross-artifact reasoning at the medium tier yet remain below the
interactive expert ceiling on compound-violation hard tasks; the
benchmark therefore continues to discriminate at frontier capability
levels.  Per-seed root-cause analysis further revealed that v1 mean
scores were substantially suppressed by fixable script bugs, not model
limits --- a methodological lesson for future agent benchmark reports.
We hope this revised release catalyses research at the intersection of
AI safety, scientific integrity, and agent evaluation.
